...
当前进度：**阶段 4（打磨与 Release）** —— `/compact` 上下文压缩（"摘要 + 按体积封顶的保留尾部"，token 计量双轨：真实 usage 展示、字符估算决策，估算超阈值自动触发）+ 内建命令从 if/else 重构为命令表。此前：阶段 3 反思与计划（`/plan` 计划模式，"先计划再动手"由程序侧硬约束保障）；阶段 2 文件工具 + 安全（read/write/list/edit、路径逃逸防护、敏感文件闸、y/n/a 权限确认带 diff 预览）；阶段 1 工具框架 + agent 主循环（`/tools` 切换 agent/纯聊天模式）。
...
REPL 内命令（`/help` 可随时查看，从命令表自动生成）：`/exit` `/clear` `/usage` `/compact` `/provider [名称]` `/tools [on|off]` `/plan <任务>`
...
├── repl.ts              # REPL 主循环（命令表分发 + 压缩调度 + 计划模式流程）
...
├── compact.ts           # 阶段 4 上下文压缩（token 估算 + 摘要替换 + 保留尾部）
└── providers/
    ├── index.ts         # 工厂
